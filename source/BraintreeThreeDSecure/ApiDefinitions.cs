...
namespace BraintreeThreeDSecure
{
	...
	interface BTConfiguration_ThreeDSecure
	{
		...
		unsafe string CardinalAuthenticationJWT();
	}
	...
	interface BTPaymentFlowDriver_ThreeDSecure
	{
		...
		unsafe void PrepareLookup (BTPaymentFlowRequest request, Action<string, NSError> completionBlock);
		...
		unsafe void InitializeChallengeWithLookupResponse (string lookupResponse, BTPaymentFlowRequest request, Action<BTPaymentFlowResult, NSError> completionBlock);
	}
	...
	interface BTThreeDSecureRequest : IBTPaymentFlowRequestDelegate
	{
		...
		unsafe BTThreeDSecureV2UICustomization V2UICustomization { get; set; }
